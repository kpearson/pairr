class MatchesController < ApplicationController
  ...
  def new
    @match = ApplicationHelper.matches(current_user).pop
  end

  def create
    Match.create(matcher_id: current_user.id,
                 matchee_id: params[:user_id],
                 rejected: params[:reject])

    if params[:reject] == "false" # && User.matchees_selected(User.matchers(current_user)).find(params[:user_id])
      redirect_to root_path(match_id: params[:uesr_id]), notice: "Your a match!!! Take a monent to email your new pair!"
    else
